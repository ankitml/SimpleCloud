...
class Responder(threading.Thread):
	...
	def run(self):
		...
		while not self.stop_event.is_set():
			try:
				...
				if side == "remote": # I received this message and should respond
					response = self.get_response(message, channel_id)
				elif side == "local": # I am supposed to send this message
					response = message
				if response:
	...
	def handle_event(self, message, channel_id):
		...
		#print("The path you were looking for is "+str(path))
		if os.path.exists(path):
			with open(path, "rb") as unpatched:
				instructions,request = zsync.zsync_delta(unpatched, hashes, num_blocks)
			# save instructions + request in Index
		else:
			instructions,request = zsync.get_instructions(hashes, num_blocks)
	...
	def handle_request_delta(self, message, channel_id):
		...
		with open(path, "rb") as stream:
			# WARNING - this returns all blocks at once. Not fun if your file is a 7GiB movie
			blocks = [b for b in zsync.get_blocks(stream, delta)]
		response = {
			"action": "delta",
			"id": 17, #message["id"],
			"path": path,
